...
export default function UploadContributionPage() {
  ...
  const [steps, setSteps] = useState<UploadStep[]>([{ id: "step-1" }]);
  ...
  function addStep() {
    ...
    setSteps((current) => [...current, { id: `step-${nextStepRef.current}` }]);
  }
  ...
  function handleFileChange(id: string, event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) {
      delete previewUrlsRef.current[id];
      setSteps((current) => current.map((item) => (item.id === id ? { id } : item)));
      return;
    }
    ...
    setSteps((current) => (
      current.map((item) => (item.id === id ? { ...item, previewUrl: url, fileName: file.name } : item))
    ));
  }
  ...
  async function submitManual(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      console.error("manual upload failed", response.status);
      return;
    }
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-8 px-6 py-10">
      <div>
        ...
        <h1 className="mt-2 text-4xl font-bold text-valorant-text">注入 Lineup</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-valorant-muted">
          选择一种注入方式：直接上传步骤图，或从视频中添加多个关键帧节点与备注。
        </p>
      </div>

      <div className="flex w-fit rounded-2xl border border-white/10 bg-black/30 p-1">
        <button
          type="button"
      ...
      {activeTab === "manual" ? (
        <form onSubmit={submitManual} className="flex flex-col gap-6 rounded-3xl border border-white/10 bg-valorant-panel/80 p-6 shadow-2xl shadow-black/30">
          ...
          <section className="flex flex-col gap-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {steps.map((step, index) => (
                <article key={step.id} className="overflow-hidden rounded-2xl border border-white/10 bg-black/20">
                  <label className="group relative flex min-h-64 cursor-pointer overflow-hidden border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(255,70,85,0.16),transparent_42%),linear-gradient(135deg,rgba(255,255,255,0.08),rgba(0,0,0,0.32))] p-4 text-sm text-valorant-muted transition hover:text-valorant-text">
                    {step.previewUrl ? (
                      <>
                        <div
                          role="img"
                      ...
                      <div className="absolute inset-4 rounded-xl border border-dashed border-white/15 bg-black/20" />
                    )}
                    ...
                    <div className="relative z-10 flex h-full w-full flex-col justify-between">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        {steps.length > 1 && (
                          <button
                            ...
                            className="rounded-lg bg-black/50 px-2 py-1 text-xs text-white/70 hover:text-valorant-red"
                          >
                            删除
                          </button>
                        )}
                      ...
                      <div>
                        <p className="text-lg font-bold text-white drop-shadow">{step.previewUrl ? "更换图片" : "选择图片"}</p>
                        <p className="mt-2 line-clamp-1 text-xs text-white/65">{step.fileName ?? "上传后这里会显示预览"}</p>
                      </div>
                    </div>

                    <input name="images" type="file" accept="image/*" className="sr-only" onChange={(event) => handleFileChange(step.id, event)} required />
                  </label>
                  <textarea
                    name="notes"
                    className="min-h-28 w-full resize-y bg-black/30 px-4 py-3 text-sm text-valorant-text outline-none placeholder:text-valorant-muted"
                    placeholder={`步骤 ${index + 1} 备注：站位、瞄准参照、背景说明或落点效果`}
                  />
                </article>
              ))}
            </div>
          ...
          <button className="cursor-pointer rounded-xl bg-valorant-red px-6 py-4 font-bold text-white hover:shadow-neon" type="submit">提交 Lineup</button>
        </form>
      ) : (
        <form onSubmit={submitVideoFrames} className="flex flex-col gap-6 rounded-3xl border border-white/10 bg-valorant-panel/80 p-6 shadow-2xl shadow-black/30">
          <section className="flex flex-col gap-4">
            <div>
