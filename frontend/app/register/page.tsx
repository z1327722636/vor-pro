...
export default function RegisterPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(response.status === 409 ? "这个邮箱已经注册，请直接登录或换一个邮箱" : `注册失败（HTTP ${response.status}）`);
        return;
      }
  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-6 py-12">
      <form onSubmit={submit} className="flex w-full max-w-md flex-col gap-5 rounded-3xl border border-white/10 bg-valorant-panel/90 p-8 shadow-2xl">
        <h1 className="text-3xl font-bold text-valorant-text">注册</h1>
        ...
        <button className="cursor-pointer rounded-xl bg-valorant-red px-5 py-3 font-bold text-white hover:shadow-neon" type="submit">创建账号</button>
        <Link href="/login" className="text-sm text-valorant-muted hover:text-valorant-blue">已有账号？去登录</Link>
      </form>
    </div>
  );
}
